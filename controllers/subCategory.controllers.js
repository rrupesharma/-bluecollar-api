...
const getAll = async (req, res)=>{
    try {
        ...
        let query = `select sg.*,i.img_name ,i.img_name2
        from public.myob_stock_group2 sg 
        left join public.images_tbl i on i.ext_id = sg.groupno and i.img_type = 'subcategory'
        where 1=1 ${key}
        order by sg.id limit ${limit} offset ${offset}`;

        let result =  await pool.executeQueryWithMsg(query,[],'No records available.')
        query = `select count(sg.id) as cnt from public.myob_stock_group2 sg 
        left join public.images_tbl i on i.ext_id = sg.groupno and i.img_type = 'subcategory'
        where 1=1 ${key}`;
...
const update = async (req, res)=>{
    try {
        ...
        let query = `UPDATE public.myob_stock_group2
        SET m__image='${image}', m__image2='${image2}', m__background='${background}', m__userid=${userid}
        WHERE id = ${id}`;
