...
async function getSaleOrderLine(id,sfid) {
  return new Promise(async function (resolve, reject) {
    try {
      let m__sf_order_id = sfid!=null ? `or m__sf_order_id = '${sfid}'` :'';
      let result = await pool.executeQuery(`select * from public.myob_salesorderline where salesorder_id=${id} ${m__sf_order_id};`, []);
      resolve(result.rows);
    } catch (error) {
      reject(error)
    }
